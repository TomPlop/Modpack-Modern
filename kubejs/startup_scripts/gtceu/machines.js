...
const registerGTCEuMachines = (event) => {
	...
	//#region Large Solar Pannel - From Sky of Grind for the code thanks a lot
        ...
        .pattern(definition => FactoryBlockPattern.start()
            .aisle('P     P', 'P     P', 'P     P', 'P     P', 'PKKKKKP', 'V V V V')
            .aisle('       ', '       ', '       ', '       ', 'KIIIIIK', '       ')
            .aisle('  PLP  ', '  PLP  ', '  RRR  ', '  RRR  ', 'KIIIIIK', 'V V V V')
            .aisle('  LCL  ', '  LCL  ', '  RGR  ', '  RGR  ', 'KIIGIIK', '       ')
            .aisle('  PXP  ', '  PLP  ', '  RRR  ', '  RRR  ', 'KIIIIIK', 'V V V V')
            .aisle('       ', '       ', '       ', '       ', 'KIIIIIK', '       ')
            .aisle('P     P', 'P     P', 'P     P', 'P     P', 'PKKKKKP', 'V V V V')
            .where('X', Predicates.controller(Predicates.blocks(definition.get())))
			...
			.where('V', Predicates.blocks('ad_astra:solar_panel'))
			.where('P', Predicates.blocks('ad_astra:iron_panel'))
			.where('I', Predicates.blocks('ad_astra:iron_plating'))
            ...
            .or(Predicates.blocks('gtceu:ulv_input_bus').setExactLimit(2))
            )
			.where('#', Predicates.air())
			.where(' ', Predicates.any())
            .build()
